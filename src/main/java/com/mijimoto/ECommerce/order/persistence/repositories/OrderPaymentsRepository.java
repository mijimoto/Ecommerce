...
import com.mijimoto.ECommerce.order.persistence.entities.OrderPayments;

/**
 * Spring Data JPA repository for entity "OrderPayments" <br>
 *
...
public interface OrderPaymentsRepository extends JpaRepository<OrderPayments, Long> {
    ...
    // List<OrderPayments> findByXxx(String xxx);

    // Example: Find by a field with LIKE behavior
    // List<OrderPayments> findByXxxContaining(String xxx);

    // Example: Find by a numerical field
    // List<OrderPayments> findByYyy(BigDecimal yyy);

    // Example: Find by foreign key (like PartID)
    // List<OrderPayments> findByPartID(Integer partID); // Use Optional<> or single result for OneToOne
